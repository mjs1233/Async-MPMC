...
#include "memory/SegregatedMemory.hpp"

namespace async_mpmc::memory::__details {
    explicit FreeTable::FreeTable(size_t alloc_count) {
    ...
    uint64_t FreeTable::check_free_count() const {
        return free_count;
    }

    uint64_t FreeTable::acquire() {
    ...
    void FreeTable::release(uint64_t index) {
        if (index >= capacity) {
            return;
        }
    ...
    explicit SegregatedMemory::SegregatedMemory(size_t alloc_size, size_t alloc_count) :
    ...
    size_t SegregatedMemory::get_alloc_size() const {
        return m_alloc_size;
    }

    bool SegregatedMemory::acquire(void** ptr) {
    ...
    void SegregatedMemory::release(void* ptr) {

        uintptr_t offset = reinterpret_cast<uintptr_t>(ptr) - reinterpret_cast<uintptr_t>(m_memory_ptr.get());
